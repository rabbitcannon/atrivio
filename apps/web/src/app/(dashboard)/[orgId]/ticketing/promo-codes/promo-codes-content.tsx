...
export function PromoCodesContent({ orgId }: PromoCodesContentProps) {
  const { toast } = useToast();
  const { currentOrg } = useOrg();

  // Check if user can manage promo codes (not box_office or finance - they have read-only access)
    ...
    try {
      if (editingCode) {
        await apiClient.patch(
          `/organizations/${orgId}/promo-codes/${editingCode.id}`,
          payload
        );
        toast({ title: 'Promo code updated' });
      } else {
        await apiClient.post(`/organizations/${orgId}/promo-codes`, payload);
  ...
  async function handleToggleActive(code: PromoCode) {
    try {
      await apiClient.patch(`/organizations/${orgId}/promo-codes/${code.id}`, {
  ...
  async function handleDelete(code: PromoCode) {
    ...
    try {
      await apiClient.delete(`/organizations/${orgId}/promo-codes/${code.id}`);
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            {canManage
              ...
              : 'View promotional discount codes.'}
          </p>
        </div>
        {canManage && (
